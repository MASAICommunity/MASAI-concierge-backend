...
Template.toolbar.helpers({
	...
	popupConfig() {
        ...
        */
				...
				//resultsFromClient = RocketChat.models.Rooms.find(query, {limit: 20, sort: {unread: -1, ls: -1}}).fetch();
				resultsFromClient = RocketChat.models.Rooms.find({$or:[{code: parseInt(filterText)}, {label: searchQuery}]}, {limit: 20, sort: {unread: -1, ls: -1}}).fetch()
				var tmp = [];
				resultsFromClient.forEach(function(itm) {
					itm.name = itm.code + ": " + itm.label;
					itm.fname = itm.code + ": " + itm.label;
					tmp.push(itm);
				});
				resultsFromClient = tmp;
				/*
