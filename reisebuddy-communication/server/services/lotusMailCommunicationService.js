...
class LotusMailCommunicationService {
	...
	constructor() {
		...
		//this.basicHeader = 'Basic ' + new Buffer('dbuser' + ':' +
		//										 'db!1@pw').toString('base64');
	...
	verifyAuthentification(authHeader) {
		console.log("username", RocketChat.settings.get('Mail_In_Reisebuddy_username'))
		console.log("password", RocketChat.settings.get('Mail_In_Reisebuddy_password'))
		console.log("my", this.basicHeader)
		console.log("given", authHeader)
	...
	parse({sender, body, subject} = {}) {
		console.log("Params: ", sender, body, subject);
		check(sender, String);
		check(body, String);
		console.log(body);
		let returnBody = body;//this.handleEncodings(body);
		...
		return {
			from: sender,
			body: decodeURIComponent(returnBody.trim())
		};
	}
	...
	send({to, message, subject} = {}) {
		...
		if (to.match(REGEX_PHONE)) {
			...
			console.log("===================")
			console.log(to);
			console.log(to.replace(/^\+/, '00').replace(/\s/, '') );
			console.log(self.baseAddress);
			console.log(self.lotusEndpoint);
