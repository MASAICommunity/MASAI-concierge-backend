...
Template.messages.helpers({
	showDepartments() {
		return Livechat.enableClose && Livechat.allowSwitchingDepartments && Department.find({ showOnRegistration: true }).count() > 1;
	},
	messagecolor() { 
		if (this.u && this.u._id === visitor.getId()) {
			return Livechat.colorguest ? "color: "+Livechat.colormessage2+"; background-color: "+Livechat.colorguest+";" : "";
		} else {
			return Livechat.coloragent ? "color:  "+Livechat.colormessage+"; background-color: "+Livechat.coloragent+";" : "";
		}
	},
	...
	agent() {
		return Livechat.agentName ? Livechat.agentName : 'Reisebuddy';
	},
	
	welcomeMessage() {
		return Livechat.welcomeMessage;
	},
	agentLogo() {
		return Livechat.agentLogo ? Livechat.agentLogo : 'https://upload.wikimedia.org/wikipedia/commons/d/d5/Deutsche_Bahn_AG-Logo.svg';
	},
	startmessage() {
		if (Livechat.startmessage && Livechat.startmessage != '') {
			return Livechat.startmessage;
		}
		return null;
	},
	messages() {
