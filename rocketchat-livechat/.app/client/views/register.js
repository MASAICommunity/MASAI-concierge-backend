...
Template.register.helpers({
	...
	hideemail() {
		return Livechat.registerwithoutmail == 1;
	},
	hidename() {
		return Livechat.withname != 1;
	},
	nicknameplaceholder() {
		return Livechat.nicknameplaceholder;
	},
	error() {
	...
	showNameFieldRegisterForm() {
		return Livechat.withname;
	},
	showEmailFieldRegisterForm() {
		return Livechat.registerwithmail; 
	},
	getName() {
...
Template.register.events({
	'submit #livechat-registration'(e, instance) {
		...
		if (Livechat.withname == 1 && $name.val().trim().length <= 0) {
			return instance.showError(TAPi18n.__('Please_fill_name_and_email'));
		} else
		if (Livechat.registerwithmail && $email.val().trim().length<=0 ) {
			return instance.showError(TAPi18n.__('EMail ist Pflicht'));
		} else if (!instance.validateForm(form, fields)) {
...
Template.register.onCreated(function() {
	...
	this.validateForm = (form, fields) => {
		const valid = fields.every((field) => form.elements[field]==null ||  !_.isEmpty(s.trim(form.elements[field].value)));
